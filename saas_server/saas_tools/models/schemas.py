...
class ToolType(str, Enum):
    """Tipos de tool disponíveis (agendamento, mensagem, encerramento; documento tratado como mensagem com arquivo no n8n)."""
...
class AssistantProfileUpsert(BaseModel):
    ...
    voice_speed: float = 1.0  # Velocidade da voz (VAPI): 0.5–2.0, 1.0 = normal
...
class FirstMessageUpdate(BaseModel):
    first_message: str = ""


class VoiceSpeedUpdate(BaseModel):
    """Velocidade da fala (assistentes.voice_speed): 0.5 a 2.0, 1.0 = normal."""
    voice_speed: float = 1.0


class PromptMasterUpdate(BaseModel):
    """Corpo para atualizar o Prompt Master (prompt_voz) do assistente."""
    prompt_voz: str = ""
...
class FlowBlockUpsert(BaseModel):
    ...
    route_context: Optional[Dict[str, Any]] = None  # { parent_router_block_key, route_key, route_position: first|middle|last }
    canal: Optional[str] = None  # 'voz' | 'whatsapp' | null (config blocks shared)
...
class SaveFlowPayload(BaseModel):
    ...
    canal: Optional[str] = None  # 'voz' | 'whatsapp' — quando setado, só substitui blocos desse canal
...
class ConvertToWhatsAppPayload(BaseModel):
    tenant_id: str = ""
