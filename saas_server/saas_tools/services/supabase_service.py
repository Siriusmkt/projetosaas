...
class SupabaseService:
    ...
    def get_tools_by_tenant(self, tenant_id: str, assistant_id: Optional[str] = None) -> List[Dict[str, Any]]:
        try:
            query = (
    ...
    # ============================================================================
    # Assistants: info (nome + foto para flow editor), profiles / flows
    # ============================================================================
    def get_assistant_info(self, assistant_id: str) -> Dict[str, Optional[str]]:
        """
        Busca nome e foto do assistente na tabela assistentes (para exibir no flow editor).
        Colunas suportadas: name/nome para nome; url_perfil/avatar_url/avatar_image para foto.
        """
        out: Dict[str, Optional[str]] = {"name": None, "photo_url": None}
        if not assistant_id:
            return out
        client = self._require_client()
        for table in ("assistentes", "assistents", "assistants"):
            try:
                # Tentar por assistente_id e por id
                for col, val in [("assistente_id", assistant_id), ("id", assistant_id)]:
                    try:
                        resp = (
                            client.table(table)
                            .select("*")
                        ...
                        if resp.data and len(resp.data) > 0:
                            ...
                            out["photo_url"] = (
                                ...
                                or row.get("avatar_image")
                                or out["photo_url"]
                            )
                            if out["name"] is not None or out["photo_url"] is not None:
                                return out
                    except Exception:
                        continue
            except Exception as e:
                logger.debug(f"get_assistant_info table={table}: {e}")
                continue
        return out
